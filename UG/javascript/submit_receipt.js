...
const submit_receipt = async (form) => {
  ...
  try {
    const response = await fetch(
      ...
      "https://crescentpipsltdbackend-production.up.railway.app/api/user/deposit/complete",
      {
        method: "POST",
...
document.querySelector("#submit").onclick = () => {
  const receipt = document.querySelector("#transaction_hash");
  if (!receipt.value)
    return (receipt.style.border = "2px solid red");
