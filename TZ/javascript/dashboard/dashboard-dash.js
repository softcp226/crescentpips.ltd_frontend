const setText = (user) => {
  ...
  document.querySelector("#final_balance").innerHTML = `TZS${user.final_balance
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;
  document.querySelector("#profit_loss").innerHTML = `TZS${user.profit_loss
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;
  document.querySelector(
    "#active_investment"
  ).innerHTML = `TZS${user.active_investment
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;

  document.querySelector("#referral_bonus").innerHTML = `TZS${user.referral_bonus
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;
