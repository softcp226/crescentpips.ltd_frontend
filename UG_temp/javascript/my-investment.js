...
let handle_cancel_investment = async (button, investment) => {
  try {
    ...
    const response = await fetch(
        ...
        "https://crescentpipsltdbackend-production.up.railway.app/api/user/investment/cancel",
      {
        method: "POST",
...
const createAndAppendElement = (element) => {
  ...
  AMTH4.innerHTML = `UGX${element.amount
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;
  ...
  EPH4.innerHTML = `UGX${element.pending_profit.toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.0`;;
...
(async () => {
  ...
  try {
    const response = await fetch(
      "https://crescentpipsltdbackend-production.up.railway.app/api/user/investments/fetch",
